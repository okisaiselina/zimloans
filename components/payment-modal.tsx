...
export function PaymentModal({ isOpen, onClose, loanPackage }: PaymentModalProps) {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "processing" && invoiceId) {
      // Start countdown timer (2 minutes for M-Pesa)
      ...
      pollInterval = setInterval(async () => {
        try {
          const response = await fetch("/api/check-payment-status", {
            ...
            body: JSON.stringify({ invoiceId, applicationId }),
          })
  ...
  }, [paymentStatus, invoiceId, applicationId, toast])
          ...
          <>
            ...
            <p className="mb-6 text-sm text-gray-600">
              ...
              <span className="font-semibold text-[#3d7c47]">
                KSH {loanPackage.ksh_equivalent}
              </span>{" "}
              processing fee via M-Pesa for your{" "}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {paymentStatus === "initiating" ? (
                  ...
                  `Pay KSH ${loanPackage.ksh_equivalent} via M-Pesa`
                )}
              </button>
            </form>
          </>
